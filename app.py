...
#  Python Dash app  |  app.py  (v3 — revenue targets + trend chart)
#
...
# ── Seed data (rev_target added) ─────────────────────────────
AGENTS = [
    dict(id=1,  name="Amara Okafor",          region="FCT / North-Central", state="FCT Abuja",  lat=9.0579,  lon=7.4951,  status="Active",   star=True,  rev=132092776, rev_target=120000000, rec="Protect & expand — top performer nationally"),
    dict(id=2,  name="Tunde Adeyemi",          region="South-West",          state="Lagos",      lat=6.5244,  lon=3.3792,  status="Active",   star=False, rev=62000000,  rev_target=90000000,  rec="Strengthen pipeline — currently below target"),
    dict(id=3,  name="Funmi Olatunji",         region="South-West",          state="Oyo",        lat=7.3775,  lon=3.9470,  status="Active",   star=False, rev=28000000,  rev_target=35000000,  rec="Growing — Ibadan C&I opportunity untapped"),
    dict(id=4,  name="VACANT — Priority hire", region="North-West",          state="Kano",       lat=12.0022, lon=8.5920,  status="Priority", star=False, rev=0,         rev_target=50000000,  rec="High potential — agro & textile C&I sector"),
    dict(id=5,  name="Ibrahim Musa",           region="North-Central",       state="Kaduna",     lat=10.5222, lon=7.4383,  status="Active",   star=False, rev=22000000,  rev_target=30000000,  rec="Steady but low volume — needs pipeline support"),
    dict(id=6,  name="VACANT — Priority hire", region="South-South",         state="Rivers",     lat=4.8156,  lon=7.0498,  status="Priority", star=False, rev=0,         rev_target=60000000,  rec="Strong Niger Delta history — reactivate urgently"),
    dict(id=7,  name="Chukwudi Eze",           region="South-South",         state="Delta",      lat=5.5174,  lon=5.7536,  status="Active",   star=False, rev=18000000,  rev_target=25000000,  rec="Rebuild trust — roof complaint history (Warri)"),
    dict(id=8,  name="Ngozi Obi",              region="South-East",          state="Enugu",      lat=6.4584,  lon=7.5464,  status="Active",   star=False, rev=19000000,  rev_target=25000000,  rec="Consistent — education sector opportunity"),
    dict(id=9,  name="VACANT",                 region="South-East",          state="Imo/Abia",   lat=5.5500,  lon=7.4833,  status="Vacant",   star=False, rev=0,         rev_target=20000000,  rec="SME density high — consider hiring Q1 2027"),
    dict(id=10, name="VACANT",                 region="North-West",          state="Sokoto",     lat=13.0059, lon=5.2476,  status="Vacant",   star=False, rev=0,         rev_target=15000000,  rec="Rural SHS potential — defer to Q4 2027"),
    dict(id=11, name="Biodun Faleyimu",        region="South-West",          state="Ondo",       lat=7.2500,  lon=5.1950,  status="Active",   star=False, rev=14000000,  rev_target=20000000,  rec="Small pipeline — referral-based only"),
    dict(id=12, name="VACANT",                 region="North-East",          state="Borno",      lat=11.8311, lon=13.1510, status="Vacant",   star=False, rev=0,         rev_target=10000000,  rec="Security constraints — defer to 2028"),
]
...
def build_target_chart(df):
    ...
    fig.update_layout(
        title=dict(text="Revenue attainment vs. target (%)", font_size=13,
                   font_color="#1A5276", x=0),
...
def build_trend_chart(data):
    ...
    for name, values in TREND_DATA.items():
        ...
        if target:
            fig.add_trace(go.Scatter(
                x=MONTHS, y=[target]*6,
                mode="lines", name=f"{name.split()[0]} target",
...
def build_table(df):
    ...
    for _, a in df.iterrows():
        ...
        pct_display = (
            html.Span(f"{pct}%",
                style={"background": attainment_color(pct),
        ...
        rows.append(html.Tr([
            ...
            html.Td([
                dbc.Button("✏", id={"type":"btn-edit","index":a["id"]},
                           color="link", size="sm",
                           style={"color":"#1A5276","fontSize":"14px","padding":"0 6px"}),
                dbc.Button("✕", id={"type":"btn-del","index":a["id"]},
                           color="link", size="sm",
...
app.layout = dbc.Container([
    dcc.Store(id="store",      data=AGENTS),
    ...
    dbc.Modal([
        ...
        dbc.ModalBody([
            ...
            dbc.Row([
                ...
                dbc.Col([dbc.Label("Star performer?", style={"fontSize":"12px","visibility":"hidden"}),
                         html.Div(id="f-attain-preview", style={"fontSize":"12px","marginTop":"6px","color":"#555"})], width=6),
            ], className="mb-2"),
            dbc.Row([
...
def refresh(data, status_filter, search):
    ...
    portfolio_pct = attainment(total_rev, total_target)
    pct_str = f"{portfolio_pct}%" if portfolio_pct else "—"
    pct_color = attainment_color(portfolio_pct)
    return (
        build_figure(df),
...
def preview_attainment(rev, target):
    if not rev or not target or target == 0:
        return ""
    pct = round(rev / target * 100, 1)
    col = attainment_color(pct)
...
def save_agent(_, data, editing_id, name, state, region, status,
    ...
    agents    = list(data)
    star_bool = (str(star) == "True")
    rev_val   = int(rev or 0)
    tgt_val   = int(rev_target or 0)
    if editing_id is not None:
        for a in agents:
            if a["id"] == editing_id:
                a.update(name=name, state=state, region=region, status=status,
                         star=star_bool, rev=rev_val, rev_target=tgt_val,
                         lat=lat, lon=lon, rec=rec or "")
    else:
        new_id = max(a["id"] for a in agents) + 1
        agents.append(dict(id=new_id, name=name, state=state, region=region,
                           status=status, star=star_bool, rev=rev_val,
                           rev_target=tgt_val, lat=lat, lon=lon, rec=rec or ""))
    return agents, False, ""
...
def confirm_delete(_, del_id, data):
    if del_id is None: return no_update, False
    return [a for a in data if a["id"] != del_id], False
